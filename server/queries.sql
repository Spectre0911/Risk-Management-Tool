...
select projectinfo.projectid, projectname, userid, opened, taskstodo, deadline, extract(day from (deadline - current_date)) as daysleft from (((select projectid from userproject where userid = <userid here> and not ismanager) as userprojects natural join (select userid, projectid from userproject where ismanager) as managers) as projectmanagers natural join projects) as projectinfo inner join (select projectid, count(taskid) as taskstodo from features inner join tasks on features.featureid = tasks.featureid group by projectid) as projecttasks on projectinfo.projectid = projecttasks.projectid;
